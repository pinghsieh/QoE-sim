function [D_MW, D_PF, D_HDR, D_NOVA, D_MW_r, Z_MW, Z_PF, Z_HDR, Z_NOVA, Z_MW_r, W_NOVA, X_PF, DALL_MW, InsRate, AvgRate_MW, AvgRate_PF, AvgRate_HDR, AvgRate_NOVA, AvgRate_MW_r] = qoefade(N, Ttot, Pch, qn, wn, Roots, alpha, beta)
...
for t=1:Ttot
    for i=1:N
        y=rand;
        if y<=Pch(i,1)
            InsRate(i,t) = 0;         % Rate = 0
        else
            if y<=(Pch(i,1)+Pch(i,2))
                InsRate(i,t) = 1;     % Rate = 1
            else
                InsRate(i,t) = 2;     % Rate = 2
            end
        end
    end
end
...
%            VQueue_MW(i,t) = InsRate(i,t)*(-X_MW(i,t))*wn(1,i);
            if ((InsRate(i,t) > 0) && ((VQueue_MW(i,t) > Dbmax) || ((VQueue_MW(i,t) == Dbmax) && (X_MW(i,t) < X_MW(head,t)))))
                Dbmax = VQueue_MW(i,t);
                head = i;
            end
        end
...
%    z = rand;
    ...
    for i=1:N
        ...
        X_HDRsh(i, t+1) =  nthroot(subplus(-X_HDR(i, t+1)-alpha(1,i)*nthroot(t^3, 4)-beta(1,i)*nthroot(t, 2)), 1);
    end
    if head > 0 && best > 0 && InsRate(head,t) < InsRate(best,t)
